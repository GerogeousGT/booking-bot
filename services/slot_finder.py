...
def generate_candidate_slots(
    ...
    """
    Возвращает кандидатов слотов (начало часа, будни, рабочие часы).
    Если задан period — дополнительно фильтрует по нему.
    """
    slots: list[datetime] = []
    ...
    while current <= to_dt:
        if current.weekday() < 5:  # Пн–Пт
            hour = current.hour
            # последний слот — 17:00 (заканчивается в 17:55, до 18:00)
            if WORK_HOUR_START <= hour <= WORK_HOUR_END - 1:
...
def build_search_range(
    ...
    min_start = now + timedelta(hours=MIN_HOURS_BEFORE)

    if period:
        # ищем в конкретный день в рамках периода
        start = parsed_dt.replace(hour=period[0], minute=0, second=0, microsecond=0)
        end = parsed_dt.replace(hour=min(period[1], WORK_HOUR_END - 1), minute=59, second=59, microsecond=0)
    else:
        # весь рабочий день
        start = parsed_dt.replace(hour=WORK_HOUR_START, minute=0, second=0, microsecond=0)
        end = parsed_dt.replace(hour=WORK_HOUR_END - 1, minute=59, second=59, microsecond=0)
