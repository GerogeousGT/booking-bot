...
- `/bookings` — предстоящие сессии карточками, с кнопкой отмены
- Перенос и отмена записи психологом. Перенос — одно действие: старая запись снимается,
  новая создаётся, клиент получает «перенесена с … на …»
- Отмена: с уведомлением, с причиной или тихо (если уже договорились лично).
  Событие удаляется из календаря, запись помечается `cancelled`, история сохраняется
