...
- Отмена записи психологом: событие удаляется из календаря, клиент получает уведомление
  (можно приложить причину); запись помечается `cancelled`, история сохраняется
