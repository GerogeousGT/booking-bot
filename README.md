...
- Подтверждение с Jitsi-ссылкой на видеозвонок
...
- Нестандартные запросы (вне рабочих часов) — на подтверждение одной кнопкой, запись создаётся автоматически
...
│   └── admin.py            # /bookings, умный хендлер, подтверждение нестандартного времени
├── services/
